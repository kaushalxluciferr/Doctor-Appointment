...
{
    const filterr=doctors.filter((doc)=>doc.speciality===speciality&&doc._id!==docId)
    setrelatedDoc(filterr)
}
  ...
  return relatedDoc && (
    ...
    <div className='grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-4 pt-5 w-full gap-y-6 px-3 sm:px-0'>
      {relatedDoc.slice(0,5).map((item,index)=>(
          <div onClick={()=>{navigate(`/appointment/${item._id}`); scrollTo(0,0)}} key={index} className='border border-blue-200 rounded-md overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'>
